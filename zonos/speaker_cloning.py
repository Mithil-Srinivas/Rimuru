...
class SpeakerEmbedding(nn.Module):
    def __init__(self, device: str = DEFAULT_DEVICE):
        super().__init__()
        prjdir = os.path.join(os.path.dirname(__file__), '..')
        model_path = os.path.join(prjdir, 'models/zonos_embedding.pt')
        self.device = device
        with torch.device(device):
            self.model = ResNet293_based()
            state_dict = torch.load(model_path, weights_only=True, mmap=True, map_location="cpu")
...
class SpeakerEmbeddingLDA(nn.Module):
    def __init__(self, device: str = DEFAULT_DEVICE):
        ...
        prjdir = os.path.join(os.path.dirname(__file__), '..')
        model_path = os.path.join(prjdir, 'models/zonos_embedding.pt')
        model_path_LDA = os.path.join(prjdir, 'models/zonos_embedding_LDA-128.pt')
        spk_model_path = model_path
        lda_spk_model_path = model_path_LDA
        self.device = device
        with torch.device(device):
